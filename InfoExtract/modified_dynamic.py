...
def _rnn_step(
  ...
  #logging info  
  #print('type final_outputs: {}\ntype final_state: {}'.format(type(final_output),type(final_state)))
  #print('number of final_output: {}\nnumber of final_state: {}'.format(final_output,final_state))
...
def bidirectional_dynamic_rnn(cell_fw, cell_bw, inputs, sequence_length=None,
  ...
  with vs.variable_scope(scope or "bidirectional_rnn"):
    ...
    with vs.variable_scope("fw") as fw_scope:
      output_fw, output_state_fw, all_states_fw = dynamic_rnn(
    ...
    with vs.variable_scope("bw") as bw_scope:
      ...
      tmp, output_state_bw, tmp_states_bw = dynamic_rnn(
  ...
  all_states_bw = _reverse(tmp_states_bw,seq_lengths=sequence_length,seq_dim=time_dim,batch_dim=batch_dim)
  all_states=(all_states_fw,all_states_bw)
  outputs = (output_fw, output_bw)
  output_states = (output_state_fw, output_state_bw,all_states)

  return (outputs, output_states,all_states)


def dynamic_rnn(cell, inputs, sequence_length=None, initial_state=None, encoder_all_states=None, attention_focus=1,
  ...
  with vs.variable_scope(scope or "rnn") as varscope:
    ...
    (outputs, final_state, all_states) = _dynamic_rnn_loop(
        cell,
        inputs,
        state,
    ...
    if not time_major:
      ...
      all_states = nest.map_structure(_transpose_batch_time, all_states)
...
def _dynamic_rnn_loop(cell,
  ...
  #modified element
  state_ta = tuple(_create_ta("state_%d" % i, _infer_state_dtype(dtype,state)) for i in range(len(flat_output_size)))
  ...
  if encoder_all_states is not None: #proxy condition
    with tf.variable_scope('attention'):
      max_length=encoder_all_states.get_shape()[1].value
      num_hidden=encoder_all_states.get_shape()[2].value
      hidden = tf.reshape(encoder_all_states,[tf.shape(encoder_all_states)[0],max_length,1,num_hidden]) #arg[0] is initially -1
      hidden_features = []
      attention_softmax_weights=[]
      for a in range(num_heads):
        k = tf.get_variable("AttnW_%d"% a, [1,1,num_hidden,num_hidden])
        hidden_features.append(tf.nn.conv2d(hidden,k,[1,1,1,1], "SAME"))
        attention_softmax_weights.append(tf.get_variable('W_attention_softmax_%d'%a,[num_hidden]))
      batch_attn_size = array_ops.stack([tf.shape(encoder_all_states)[0], num_hidden])
      attns = [array_ops.zeros(batch_attn_size, dtype=dtype) for _ in range(num_heads)]
      for a in attns:  # Ensure the second shape of attention vectors is set.
        a.set_shape([None, num_hidden])
    ...
    def attention(state):
      ...
      if nest.is_sequence(state):
        ...
        for state in states:
          ndims = state.get_shape().ndims
          if ndims:
            assert ndims == 2
        state = array_ops.concat(states, 1)
      for a in range(num_heads):
        with tf.variable_scope("Attention_%d" % a):
          y = tf.contrib.layers.fully_connected(inputs=state, num_outputs=num_hidden, biases_initializer=tf.constant_initializer(0.0))
          y = array_ops.reshape(y,[-1,1,1,num_hidden])
          #print('shape of y: {}\nshape of hidden_feature: {}'.format(y.get_shape(),hidden_features[a].get_shape()))
          s = math_ops.reduce_sum(attention_softmax_weights[a] * math_ops.tanh(hidden_features[a] + y),[2,3]) #should try implementing dot here
          a = tf.nn.softmax(s)
          c = tf.reduce_sum(array_ops.reshape(a, [-1, max_length, 1, 1])*hidden, [1,2])
          cs.append(array_ops.reshape(c,[-1,num_hidden]))
      return cs
	
  #this function has been modified, adding arg: state_ta
  #augmenting attention to this function
  def _time_step(time, output_ta_t, state, state_ta_t, attns=None):
    ...
    # modified element here
    if attns is not None:
      input_size = input_t.get_shape().with_rank(2)[1].value
      #need to also consider the output size, which is [N, num_class] supposedly for biclassifier
      #print('inp dtype: {}\nattns dtype: {}'.format(input_t.dtype,attns[0].dtype))
      for attn in attns:
        input_t=tf.concat([input_t,attn],1)
      x = tf.contrib.layers.fully_connected(inputs=input_t,num_outputs=input_size,biases_initializer=tf.constant_initializer(0.0))
    ...
    if attns is not None:
      ...
      with tf.variable_scope('attention_output_projection'):
        output_size = output.get_shape().with_rank(2)[1].value
        for attn in attns:
          output=tf.concat([output,attn],1)
        output= tf.contrib.layers.fully_connected(inputs=output,num_outputs=output_size,biases_initializer=tf.constant_initializer(0.0))
    ...
    #modified lines
    states = nest.flatten(new_state)
    state_ta_t = tuple(ta.write(time,substate) for ta, substate in zip(state_ta_t,states))

    result =  (time + 1, output_ta_t, new_state, state_ta_t, attns) if attns is not None else (time + 1, output_ta_t, new_state, state_ta_t)
    return result

  if encoder_all_states is not None:
    _, output_final_ta, final_state, all_states_ta, attns = control_flow_ops.while_loop(
      cond=lambda time, *_: time < time_steps,
      body=_time_step,
      loop_vars=(time, output_ta, state, state_ta, attns),
      parallel_iterations=parallel_iterations,
      swap_memory=swap_memory)
  else:
  ...
  #logging info  
  #print('unpacked final_output: {}'.format(final_outputs))
  ...
  #logging info  
  #print('restored final_output: {}'.format(final_outputs))
  
  final_outputs = nest.pack_sequence_as(
      structure=cell.output_size, flat_sequence=final_outputs)

  #modified
  all_states=tuple(ta.stack() for ta in all_states_ta)
  for this_state,this_state_size in zip(all_states,flat_state_size):
    shape = _state_size_with_prefix(this_state_size,prefix=[const_time_steps,const_batch_size])
    this_state.set_shape(shape)
  all_states = nest.pack_sequence_as(structure=flat_state_size[-1],flat_sequence=all_states)

  #logging info  
  #print('type of all_states: {}\ntype of all_output: {}\ntype of final_state: {}'.format(type(all_states),type(final_outputs),type(final_state)))
  #print('all_states: {}\nall_output: {}\nfinal_state: {}'.format(all_states,final_outputs,final_state))
  return (final_outputs, final_state, all_states)
